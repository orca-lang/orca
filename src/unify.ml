...
let unify sign flex e1 e2 =
  ...
  and unify_many sign es1 es2 =
    let unify_each sigma1 e1 e2 =
      sigma1 @ unify (subst_list sigma1 e1) (subst_list sigma1 e2)
    in
  ...
  and unify_pi sign tel1 t1 tel2 t2 =
    ...
    | [], [] -> unify t1 t2
    | tel1, [] -> unify (Pi (tel1, t1)) t2
    | [], tel2 -> unify t1 (Pi (tel2, t2))
    | (_, n1, e1)::tel1, (_, n2, e2)::tel2 ->
       let sigma = (n1, Var n2) :: (unify e1 e2) in
